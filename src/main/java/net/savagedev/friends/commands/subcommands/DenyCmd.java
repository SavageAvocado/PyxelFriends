...
public class DenyCmd extends SubCommand {
    public DenyCmd(PyxelFriends plugin, Permission permission) {
    ...
    public void execute(CommandSender user, String... args) {
        ...
        if (args.length == 1) {
            this.getPlugin().getMessageUtil().message(user, this.getPlugin().getConfig().getString("messages.invalid-arguments").replace("%0%", "friend deny <name>"));
            return;
        }
        ...
        if ((potentialFriend = this.getPlugin().getServer().getOfflinePlayer(args[1])) == null || !potentialFriend.hasPlayedBefore()) {
        ...
        request.deny(this.getPlugin());

        for (String line : this.getPlugin().getConfig().getStringList("messages.request-denied"))
            this.getPlugin().getMessageUtil().message(user, line.replace("%0%", potentialFriend.isOnline() ? Bukkit.getPlayer(args[1]).getDisplayName() : potentialFriend.getName()));
    }
}
